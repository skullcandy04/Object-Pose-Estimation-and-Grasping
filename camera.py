...
class Camera(object):
    ...
    def compute_camera_matrix(self):
        ...
        intrinsic_matrix = np.array([[self.focal_length,0,self.fov_width/2],
                                     [0,self.focal_length,self.fov_height/2],
                                     [0,0,1]])
        fov = np.pi * self.fov_width / 180
        projection_matrix = np.array(p.computeProjectionMatrixFOV(fov, self.fov_width/self.fov_height, self.near, self.far))  # TODO: compute projection_matrix from camera parameters

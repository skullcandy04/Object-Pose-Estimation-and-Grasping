...
class RGBDataset(Dataset):
    def __init__(self, dataset_dir, has_gt):
        ...
        self.dataset_length = len(os.listdir(dataset_dir+'rgb/'))
    ...
    def __getitem__(self, idx):
        ...
        rgb_img_filename = os.listdir(self.dataset_dir + 'rgb/')[idx]
        rgb_img = image.read_rgb(os.path.join(self.dataset_dir + 'rgb/', rgb_img_filename))
        if self.has_gt:
            gt_img_filename = os.listdir(self.dataset_dir + 'gt/')[idx]
            gt_mask = image.read_mask(os.path.join(self.dataset_dir + 'gt/', gt_img_filename))
            gt_mask = torch.from_numpy(gt_mask).long()
            sample = {'input': self.transform(rgb_img), 'target': gt_mask}
        else:
            sample = {'input': self.transform(rgb_img)}
        return sample
